...
public:
  float angles[3]; // Contains the joint angles along the kinematic chain, in
                   // degrees
  //(corrected) link lengths  (edit Vincent: had to move the declaration here,
  ...
  HTransform T0_1; // Homogenous transform from Link 0 to Link 1, updated by
                   // computeT0_1()
  HTransform T1_2; // Homogenous transform from Link 1 to Link 2, updated by
                   // computeT1_2()
  HTransform T2_3; // Homogenous transform from Link 2 to Link 3, updated by
                   // computeT2_3()
  HTransform T3_E; // Homogenous transform from Link 3 to end effector frame,
                   // updated by computeT3_E()
  HTransform T0_E; // Homogenous transform from Link 0 to end effector frame,
                   // updated by computeT0_E()
  float F[3]; // operational space position of the end effector (x, y, alpha),
              // updated by computeF()
  float J[3][3];  // end effector Jacobian, updated by computeJ()
  float DH[4][4]; // table of DH paramters, as laid out in the PDF
