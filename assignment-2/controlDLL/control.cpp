...
double computeTf(GlobalVariables &gv) {
  ...
  // for each joint
  double min_tf_q1 =
      max((1.5 * delta_q1) / gv.dqmax[0], sqrt((6 * delta_q1) / gv.ddqmax[0]));

  double min_tf_q2 =
      max((1.5 * delta_q2) / gv.dqmax[1], sqrt((6 * delta_q2) / gv.ddqmax[1]));

  double min_tf_q3 =
      max((1.5 * delta_q3) / gv.dqmax[2], sqrt((6 * delta_q3) / gv.ddqmax[2]));
...
void PreprocessControl(GlobalVariables &gv) {
  ...
  if ((gv.dof == 3) || (gv.dof == 6)) {
    ...
    g123[0] = g * (m3 * (l1 * c1 + l2 * s12 + r3 * s123) +
                   (l1 * c1 + r2 * s12) * m2 + r1 * c1 * m1);
    g123[1] = g * (r2 * s12 * m2 + (l2 * s12 + r3 * s123) * m3);
    g123[2] = g * r3 * s123 * m3;

    // maps the torques to the right joint indices depending on the current
    // mode:
    if (gv.dof == 3) {
      gv.G[0] = g123[0];
      gv.G[1] = g123[1];
...
void initNjtrackControl(GlobalVariables &gv) {
  ...
  CS.a2 = PrVector3(
      3 / (dt * dt) * (qd1 - q1), 3 / (dt * dt) * (qd2 - q2),
      3 / (dt * dt) * (qd3 - q3)
  );
  CS.a3 = PrVector3(
      -2 / (dt * dt * dt) * (qd1 - q1), -2 / (dt * dt * dt) * (qd2 - q2),
      -2 / (dt * dt * dt) * (qd3 - q3)
  );
}
...
void floatControl(GlobalVariables &gv) {
  ...
  // gv.tau = ?
  // this only works on the real robot unless the function is changed to use
  // cout the handed in solution must not contain any printouts PrintDebug(gv);
}
...
void jgotoControl(GlobalVariables &gv) {
  if (gv.dof == 3 || gv.dof == 6) {
    ...
    if (gv.dof == 3) {
      gv.tau[0] =
          -gv.kp[0] * (gv.q[0] - gv.qd[0]) - gv.kv[0] * gv.dq[0] + gv.G[0];
      gv.tau[1] =
          -gv.kp[1] * (gv.q[1] - gv.qd[1]) - gv.kv[1] * gv.dq[1] + gv.G[1];
      gv.tau[2] =
          -gv.kp[2] * (gv.q[2] - gv.qd[2]) - gv.kv[2] * gv.dq[2] + gv.G[2];
    } else if (gv.dof == 6) {
      gv.tau[1] =
          -gv.kp[1] * (gv.q[1] - gv.qd[1]) - gv.kv[1] * gv.dq[1] + gv.G[1];
      gv.tau[2] =
          -gv.kp[2] * (gv.q[2] - gv.qd[2]) - gv.kv[2] * gv.dq[2] + gv.G[2];
      gv.tau[4] =
          -gv.kp[4] * (gv.q[4] - gv.qd[4]) - gv.kv[4] * gv.dq[4] + gv.G[4];
    }
  }
}
...
void njtrackControl(GlobalVariables &gv) {
  if (gv.curTime <= CS.tf) {
    ...
    gv.tau[0] =
        -gv.kp[0] * (gv.q[0] - qd1) - gv.kv[0] * (gv.dq[0] - dqd1) + gv.G[0];
    gv.tau[1] =
        -gv.kp[1] * (gv.q[1] - qd2) - gv.kv[1] * (gv.dq[1] - dqd2) + gv.G[1];
    gv.tau[2] =
        -gv.kp[2] * (gv.q[2] - qd3) - gv.kv[2] * (gv.dq[2] - dqd3) + gv.G[2];
  } else {
    floatControl(gv); // Remove this line when you implement this controller
  }
}
...
void proj1Control(GlobalVariables &gv) { njtrackControl(gv); }
...
void proj2Control(GlobalVariables &gv) {
  ...
  double F2 =
      -gv.kp[2] * (gv.x[2] - alpha_traj) - gv.kv[2] * (gv.dx[2] - dalpha_traj);

  for (int i = 0; i < 3; i++) {
    gv.tau[i] = gv.Jtranspose[i][0] * F0 + gv.Jtranspose[i][1] * F1 +
                gv.Jtranspose[i][2] * F2;
  }
}
...
void proj3Control(GlobalVariables &gv) {
  ...
  auto dt = t - CS.t0;
  // movement trajectory during the 3 revolutions during the 3 phases 3 with the
  // parabolic blends
  if (dt <= 20) {
    ...
    // the linear deceleration phase needs to start at t=10s
      ...
      // the dt reverse is just a mathematical trick that allows the cos/sin to
      // continue in the correct direction and not to reset
      dt = dt - 20;
    ...
    double F0 =
        -gv.kp[0] * (gv.x[0] - x_traj) - gv.kv[0] * (gv.dx[0] - dx_traj);
    double F1 =
        -gv.kp[1] * (gv.x[1] - y_traj) - gv.kv[1] * (gv.dx[1] - dy_traj);
    double F2 = -gv.kp[2] * (gv.x[2] - alpha_traj) -
                gv.kv[2] * (gv.dx[2] - dalpha_traj);
