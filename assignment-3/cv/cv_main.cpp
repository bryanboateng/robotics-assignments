...
bool select_object =
    false; // this variable turns true when the user presses the left mouse
           // button to begin the selection of a color in the image
bool track_object = false; // this variable turns true when the user finishes
                           // selecting a color in the image, and turns false if
                           // this selection is cleared (pressing 'c')
bool estimated_histogram = false; // this variable turns true when the histogram
                                  // of the selected color has been estimated

pthread_mutex_t mutexGVIn; // mutex for accessing (r/w) gvIn;

void Sleep(
    struct timespec &startTimeOfCurrentTimeSlice,
    float dt
); // sleeps until the next iteration of the CV loop is due

///////////////////
// OpenCV variables

// OpenCV images ptr
IplImage *image = NULL, *hsv = NULL, *hue = NULL, *sat = NULL, *val = NULL,
         *mask = NULL, *backproject = NULL, *backproject_processed = NULL,
         *message_img = NULL, *s_mask = NULL, *v_mask_dark = NULL,
         *v_mask_bright = NULL, *sv_mask = NULL;
...
// If we have more dimensions we could have several values (between black and
// white) in the backprojection ->
...
void cvLoop(cmdfunc robotCmd, imgFactory getSimImg = NULL) {
  ...
  cv::Mat *simulated_frame =
      NULL; // pointer to the rendered frame (in simulation only)
#else
  cv::Mat mat_frame;
#endif
  int c;                              // Clicked keyboard
  Float previous_iter_time = 0.0f;    // Time of the last iteration of the loop
  float previous_command_time = 0.0f; // Time of the last iteration of the loop
                                      // that sent a command to the robot
  Circle crcl; // Structure to store the parameters of the detected circle in
               // the image
  struct timespec startTimeOfLastSlice = {0, 0};
  if (robotCmd == NULL)
    return;

  // Initialize the mutex that protects the r/w operations on the state variable
  // of the robot
  ...
  float diameter_desired_px; // Desired diameter of the circle in the image in
                             // pixels
  float dt; // This is suppose to be the delta time between iterations, but it
            // is just an scaling factor
  ...
  // To grab images from the real camera -> Be careful if you have several cams
  // connected!
  //    CvCapture* capture = 0;
  ...
  // The capture for the camera could not be initialized -> Is the camera
  // connected?
  //    if( !capture )
  if (!capture.isOpened()) {
  ...
  // Set the desired feature values in opencv frame (origin in the upper left
  // corner)
  ...
  initVisualServoing(
      f, img_width, img_height, diameter_real, diameter_desired_px, dt,
      desired_s_opencvf
  );
  // return; BEGIN
  std::cout << "0" << std::endl;
  ...
  std::cout << "1" << std::endl;
  cv::moveWindow(VS_WINDOW_NAME, img_width, img_height);
  std::cout << "2" << std::endl;
  cv::namedWindow(BACKPROJ_HIST_IMAGE_WINDOW_NAME, 1);
  std::cout << "3" << std::endl;
  cv::moveWindow(BACKPROJ_HIST_IMAGE_WINDOW_NAME, 2 * img_width, img_height);
  std::cout << "4" << std::endl;
  cv::namedWindow(SV_MASK_WINDOW_NAME, 1);
  std::cout << "5" << std::endl;
  cv::moveWindow(SV_MASK_WINDOW_NAME, 3 * img_width, img_height);
  std::cout << "6" << std::endl;
  cv::namedWindow(BACKPROJ_HIST_PROC_IMAGE_WINDOW_NAME, 1);
  std::cout << "7" << std::endl;
  cv::moveWindow(
      BACKPROJ_HIST_PROC_IMAGE_WINDOW_NAME, 4 * img_width, img_height
  );
  std::cout << "8" << std::endl;
  ...
  while (cv_running) {
    ...
    // Make a local copy of the robot state (protected r/w operation using the
    // mutex)
    ...
    if (simulated_frame) {
      delete simulated_frame;
    } // throw away last frame
    cv::Mat *simulated_frame = getSimImg();
    iplimg =
        cvIplImage(*simulated_frame); // Backwards compatibility, TODO: remove
    frame = &iplimg; // pointer does not actually change, just to make it clear
                     // here where it points to
    if (!frame) {
      fprintf(
          stderr, "[cvLoop] ERROR: No image could be grabbed from the camera!\n"
      );
      break;
    }
#else
    ...
    iplimg = cvIplImage(mat_frame); // Backwards compatibility, TODO: remove
    frame = &iplimg; // pointer does not actually change, just to make it clear
                     // here where it points to
#endif
    // Check that the image could be grabbed

    // In the first loop iteration and using the first grabbed image we
    // initialize the image containers (allocate all the buffers) and the
    ...
    if (track_object) // The user already selected a color to track
    {
      ...
      // Backproject the hue image (last grabbed image in hue color space) using
      ...
      cv::Mat backprj = cv::cvarrToMat(backproject);
      cv::Mat backprj_processed = cv::cvarrToMat(backproject_processed);

      // Threshold the backprojection into a black-and-white estimate of the
      // pixel belonging to the object
      cv::threshold(backprj, backprj_processed, 200, 255, cv::THRESH_BINARY);

      // Apply the combined mask: set to zero the values of the backprojection
      // that are either not colorful enough or too dark/bright
      cvSet(backproject_processed, cvScalar(0), sv_mask);

      // Also filter the binary estimate based on the pixel neighbour's
      // estimates using erode/dilate:
      //  The opening operation cleans the backprojection of small noisy pixels
      //  and completes holes in the blob More info:
      //  http://docs.opencv.org/2.4/doc/tutorials/imgproc/opening_closing_hats/opening_closing_hats.html
      cv::Mat opening_kernel = cv::getStructuringElement(
          cv::MORPH_ELLIPSE, cv::Size(9, 9), cv::Point(5, 5)
      );
      cv::morphologyEx(
          backprj_processed, backprj_processed, cv::MORPH_CLOSE, opening_kernel
      );
      ...
      cv::GaussianBlur(
          backprj_processed, backprj_processed, Size(13, 13), 4, 4
      );
      ...
      putText(
          img, framerate_msg, cvPoint(230, 228), FONT_HERSHEY_SIMPLEX, 0.3,
          CV_RGB(0, 0, 0)
      );
      ...
      if (findCircleFeature(img, backprj_processed, crcl)) {
        ...
        if (gv.x.size() == 7) {
          /////////////////////////////////////////////////////////////////
          // Compute the command to send to the robot using Visual Servoing ->
          // calls a function of cv_control
          ...
          if ((previous_command_time + COMMAND_SEND_MIN_TIME) < gv.curTime) {
            ...
            Float time_difference_between_commands =
                gv.curTime - previous_command_time;
            previous_command_time = gv.curTime;
            char framerate_cmds_msg[255];
            sprintf(
                framerate_cmds_msg, "fpsc: %.3f",
                1.0 / time_difference_between_commands
            );
            putText(
                img, framerate_cmds_msg, cvPoint(230, 238),
                FONT_HERSHEY_SIMPLEX, 0.3, CV_RGB(255, 0, 0)
            );
          }
        } else {
          fprintf(
              stderr, "[cvLoop] ERROR: We do not send any command. The robot "
                      "is not is controllable in its 6 joints!\n"
          );
        }
      }

    } else // The user did not select any object yet or a previous selection was
           // cleaned -> we are not tracking
    {
      // Pointing to the wall (quaternion(w,x,y,z):0.50, 0.50, -0.50, 0.50)
      sprintf(
          command_buf, "jgoto %.4f %.4f %.4f %.4f %.4f %.4f", 90.0, -135.0, 0.0,
          0.0, 45.0, 0.0
      );
      // Pointing to the window (quaternion(w,x,y,z):0.71, 0.0, 0.71, 0.0)
      // sprintf(command_buf,"jgoto %.4f %.4f %.4f %.4f %.4f
      // %.4f",0.0,-45.0,180.0,0.0,-45.0,0.0);
      robotCmd(command_buf);
    }
    ...
    cvLine(
        image, cvPoint((int)desired_s_opencvf[0], 0),
        cvPoint((int)desired_s_opencvf[0], image->height),
        cvScalar((10), (10), (10), 1)
    );
    cvLine(
        image, cvPoint(0, (int)desired_s_opencvf[1]),
        cvPoint(image->width, (int)desired_s_opencvf[1]),
        cvScalar((10), (10), (10), 1)
    );
...
void cvStop(void) { cv_running = false; }
...
void onMouse(int event, int x, int y, int flags, void *param) {
  ...
  switch (event) {
  case CV_EVENT_LBUTTONDOWN: // Click with the left button to begin the
                             // selection
    origin = cvPoint(x, y);
...
void Sleep(struct timespec &startTimeOfCurrentTimeSlice, float dt) {
  ...
  if (startTimeOfCurrentTimeSlice.tv_sec ==
      0) { // first time we are called, simply set startTimeOfCurrentTimeSlice
           // and return
  ...
  sleeptime.tv_sec =
      delta.tv_sec - (now.tv_sec - startTimeOfCurrentTimeSlice.tv_sec);
  sleeptime.tv_nsec =
      delta.tv_nsec - (now.tv_nsec - startTimeOfCurrentTimeSlice.tv_nsec);
  ...
  clock_gettime(
      CLOCK_MONOTONIC, &startTimeOfCurrentTimeSlice
  ); // a new timeslice has begun
  return;
}
