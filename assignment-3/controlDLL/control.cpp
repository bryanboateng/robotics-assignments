...
void GetNonsingularSelection(PrMatrix &dest, GlobalVariables &gv) {
  ...
  PrMatrix3 R35(c4 * c5, -c4 * s5, -s4, s5, c5, 0, s4 * c5, -s4 * s5, c4);
  PrMatrix3 R5 = R3 * R35;
...
void GetEscapeTorque(
    const PrVector &fPrime,
    PrVector &escapeTorque,
    GlobalVariables &gv
) {
  ...
  PrMatrix3 R4(
      c1 * c23 * c4 - s1 * s4, -c1 * c23 * s4 - s1 * c4, c1 * s23,
      s1 * c23 * c4 + c1 * s4, -s1 * c23 * s4 + c1 * c4, s1 * s23, -s23 * c4,
      s23 * s4, c23
  );
...
void OpDynamics(const PrVector &fPrime, GlobalVariables &gv) {
  ...
  gv.tau = rJacobianT * rLambda * nonsingularSelection * fPrime +
           nullSpaceT * Ainverse * escapeTorque + gv.G;
}
...
void initTrackControl(GlobalVariables &gv) { FindOperationalCubicSpline(gv); }
...
void floatControl(GlobalVariables &gv) { gv.tau = gv.G; }
...
void jgotoControl(GlobalVariables &gv) {
  ...
  for (int ii = 0; ii < gv.dof; ii++) {
    local_dqd[ii] -=
        (gv.q[ii] - gv.qd[ii]) * gv.kp[ii] / max(gv.kv[ii], MIN_KV);
...
void trackControl(GlobalVariables &gv) {
  ...
  PrVector fPrime =
      (-gv.kp * (gv.Einverse * (gv.x - local_xd)) -
